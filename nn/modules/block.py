...
class MultiResSimAM(nn.Module):
    def __init__(self, channels, e_lambda=1e-4, scales=[1, 2, 4]):
        super(MultiResSimAM, self).__init__()
        self.e_lambda = e_lambda
        self.scales = scales
        self.activation = nn.Sigmoid()
        self.residual_block = SmallResidualBlock(channels)
    ...
    def forward(self, x):
        ...
        for scale in self.scales:
            ...
            n = x_scaled.size(2) * x_scaled.size(3) - 1
            x_minus_mu_square = (x_scaled - x_scaled.mean(dim=[2, 3], keepdim=True)).pow(2)
        ...
        attention = self.activation(sum(attention_maps) / len(self.scales))

        # Apply attention and then pass through the residual block
        out = x * attention
